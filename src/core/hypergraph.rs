...
impl Hypergraph {
    ...
    pub fn get_edges(
        ...
        if order.is_none() && size.is_none() {
            if ids {
                ...
                for edge in self.edge_list.keys() {
                    let py_tuple = PyTuple::new(py, edge)?;
        ...
        match PyList::new(py, edges) {
            Ok(py_list) => {
                Ok(py_list.into_py(py))
            }
            Err(e) => {
                Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(
                    format!("Failed to create PyList: {}", e),
                ))
            }
    ...
    pub fn get_sizes(&self, py: Python) -> PyResult<PyObject> {
        ...
        match PyList::new(py, sizes) {
            ...
            Err(e) => {
                Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(
                    format!("Errore nella creazione della lista: {}", e),
                ))
            }
        }
    }
    ...
    fn get_incident_edges(
        ...
        match PyList::new(py, incident_edges) {
            Ok(py_list) => {
                Ok(py_list.into())
            }
            Err(e) => {
    ...
    pub fn get_neighbors(
        ...
        let py_neighbors = PyList::new(py, neighbors.into_iter().collect::<Vec<_>>());
