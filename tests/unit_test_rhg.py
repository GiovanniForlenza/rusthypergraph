...
def print_attr_meta(hypergraph = hx.Hypergraph):
    result = hypergraph.get_attr_meta(obj = 3, attr = "name")
    print(result)
...
def print_get_edges(hypergraph = hx.Hypergraph):
    ...
    subgraph = hypergraph.get_edges(size=3, up_to = True, subhypergraph =True)
    print(subgraph.get_nodes(metadata = False))
...
def print_get_nodes(hypergraph=hx.Hypergraph):
    nodes = hypergraph.get_nodes(metadata = False)
    print(nodes)
...
def print_get_meta(hypergraph = hx.Hypergraph):
    meta = hypergraph.get_meta(obj_id=3)
    print(meta)
...
def print_add_edge(weighted = weighted):
    hypergraph = hx.Hypergraph(weighted=weighted)
    hypergraph.add_edge(edge=(22,33), weight=2)
...
def print_add_nodes():
    hypergraph = hx.Hypergraph()
    nodes = [2,4,5,6,7,8]
    hypergraph.add_nodes(nodes=nodes)
    print(hypergraph.get_nodes(metadata = False))
    # assert : [2, 4, 5, 6, 7, 8]
...
def print_remove_edges(hypergraph = hx.Hypergraph):
    edge_list = [(1, 2), (2, 3)]
    hypergraph.remove_edges(edges = edge_list)
    print(hypergraph.get_edges())
...
def print_remove_nodes(hypergraph = hx.Hypergraph, keep_edges = bool):
    nodes = [6,7]
    print(hypergraph.get_nodes(metadata = False))
    hypergraph.remove_nodes(nodes = nodes, keep_edges = keep_edges)
    print(hypergraph.get_nodes(metadata = False))
